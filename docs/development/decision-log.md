...
**状态**：B18–B21 全部验收合并推送，D13 收口（B21 为首个 Opus 工单批次）

---

### 2026-08-18 D14 故事知识图谱：长篇一致性的派生层（B22–B26）

**背景**：负责人定向（参照 GraphRAG/LightRAG/PlotPilot 建故事图谱，要求充足调研）。
三路调研完成：外部方案（GraphRAG/LightRAG/NWM/Graphiti/核验基准）、写作产品先例
（NovelAI Lorebook/novelcrafter Codex/Sudowrite）、PlotPilot 源码（章末管线/三元组
索引/伏笔注册表）、仓库内地基（现状裸奔：章节生成从不读已完成正文；先例齐备：
独立表 + content_hash 惰性失效 + 单点注入 + 按任务类型路由）。

**决策**（已拍板部分）：图谱做成可见可编辑的「故事圣经」；抽取跟随默认模型。
架构：LightRAG 式双索引（无社区层）+ NWM 带章节戳时序状态边 + 伏笔线程一等公民 +
PlotPilot 章末抽取管线 + ConStory 图锚定定向核验；图为正文纯派生（可全量重建），
用户编辑走覆盖层。详见 `story-graph-design.md`。

**拍板结果（2026-08-19 负责人确认 4 项，D-D14-1～4）**：

- D-D14-1 MVP 粒度：实体+状态边+伏笔线程三件套先行（B22），事件时间线二期——按推荐。
- D-D14-2 核验产品语义：非阻断黄色警告，进候选自查结果区——按推荐。
- D-D14-3 检索起步形态：**偏离推荐，同期上向量**——B23 为 FTS5/别名激活 +
  向量检索（sqlite-vec + 嵌入模型走网关）的混合检索，不推二期。
- D-D14-4 视角语义：故事圣经默认上帝视角（展示到已写到的最新章），读者视角由
  按章节滑块回放覆盖——按推荐。

**状态**：已确认；B22 已实现并验收（数据层 + 抽取管线两工单，实测四触发链
全通，见 `b22-story-graph-data-design.md` §4，随行登记 TD-036 数据根绝对路径
缺陷）；B23 检索接入排队
