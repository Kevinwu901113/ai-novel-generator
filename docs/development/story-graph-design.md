...
> 可随时切换）。本文档 = 调研结论 + 架构设计 + 分批实施计划 + 拍板结果。
> 状态：**已确认**（2026-08-19 负责人拍板 §8 全部 4 项，其中检索形态偏离推荐：
> 向量检索与 FTS5 同期上，见 D-D14-3）。
...
story_events           事件时间线（二期，D-D14-1 拍板推迟，v22 不建此表）：
                       chapter 序锚定，不解析故事内绝对时间
...
- **改章 = 重抽该章 + 链接拼接失效**（D-B22-7 裁定收窄）：只删
  `source_chapter=N` 的 extracted 记录，superseded 链上的前驱边重新链接到
  最近存续后继（无则重开）——下游边锚定在未改章节的正文上，不级联删除；
  全量重建命令兜底一切增量 bug。
...
2. **种子匹配**：本章蓝图 goal + 场景计划文本，经别名表精确/FTS5 匹配命中实体，
   并叠加向量检索召回（sqlite-vec + 嵌入模型走网关，D-D14-3 拍板同期上）——
   两路结果 RRF 融合，向量层实现细节在 B23 批设计定；
...
## 8. 拍板结果（2026-08-19 负责人确认，D-D14-1～4）

1. **MVP 图粒度（D-D14-1，按推荐）**：「实体 + 状态边 + 伏笔线程」三件套先行
   （B22），事件时间线（story_events + 时间线 UI + 时序冲突核验）二期——v22
   migration 不建 story_events 表，事件层落地时另起 migration。
2. **核验产品语义（D-D14-2，按推荐）**：非阻断黄色警告，进候选自查结果区；
   不阻断采用，无一键改稿。
3. **检索起步形态（D-D14-3，偏离推荐）**：**同期上向量**——B23 = FTS5/别名
   激活 + sqlite-vec 向量召回（嵌入模型走网关按任务类型路由）双路 RRF 融合。
4. **视角语义（D-D14-4，按推荐）**：默认上帝视角（展示到已写到的最新章），
   读者视角由「按章节滑块回放」覆盖。
...
| 批次 | 内容                                                                                               | 依赖   |
| ---- | -------------------------------------------------------------------------------------------------- | ------ |
| B22  | migration v22 图数据层 + STORY_GRAPH_EXTRACT 抽取 executor + 章末触发 + 回填命令（纯后台）         | 已拍板 |
| B23  | 检索接入 loadContext（useStoryGraph 开关）：FTS5/别名 + sqlite-vec 向量双路 RRF + dogfood 双臂对照 | B22    |
| B24  | 故事圣经 UI（实体卡/状态/伏笔/待审队列 + 用户覆盖层编辑）                                          | B22    |
| B25  | 图核验 Critic 进候选自查区（非阻断黄色警告）                                                       | B22    |
