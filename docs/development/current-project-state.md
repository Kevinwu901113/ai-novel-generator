...
> 状态文档版本：10（2026-08-17）。本文档由项目负责人维护，仅在目标状态、能力矩阵或推进位置发生实质变化时更新。
...
| 代码核验范围    | apps/server、apps/web、apps/worker、apps/writing-experiment-runner、packages（含 graph 模块）、数据库 migration v1–v19、CI |
...
| **GE-3 Intake 产品 UI（B4）**                     | `apps/web/src/intake/*`、`journey/JourneyNav.tsx`、App.tsx 旅程 shell、contracts + desktop-client + server 校验表贯通的 intake.* 通道、`docs/development/b4-intake-ui-design.md`                                                                 | 四阶段旅程导航 + 对话式访谈（回答/跳过/完成/升级 Gate/失败重建）+ CreationSpec 编辑器（CAS + 显式失效级联）；旧 Grill 工作台移出默认入口（代码保留）；随行解决 TD-022/024/025-3                                                                                                                                                                                                                                                                                                                         | GE-4 起 B5/B6 逐阶段扩展旅程                                                         |
...
| Web 服务形态                                      | `apps/server`（hand-rolled node:http + smoke.mjs）、`apps/web`（前端构建产物由 server 静态托管）；决策与设计见 D11、`b11-web-server-design.md`、`b12-web-frontend-design.md`、`b13-electron-removal-design.md`                                   | 浏览器（本机 + 局域网多设备，访问令牌认证）访问；`pnpm build && pnpm start` 生产运行；ubuntu CI 冒烟（`node apps/server/scripts/smoke.mjs`）                                                                                                                                                                                                                                                                                                                                                            | 保留                                                                                 |
...
Web 应用（浏览器访问）当前实际走到（`apps/web/src/App.tsx`，2026-08-14 创作台布局重设计后）：
...
GE-9 质量增强            → 待开始
B11 Web 服务端           → ✅ COMPLETE（2026-08-17）
B12 前端迁移 apps/web    → ✅ COMPLETE（2026-08-17，含本机真机验收）
B13 Electron 删除收口    → ✅ COMPLETE（2026-08-17）
```
...
apps/web（前端）/ apps/server（传输）      → GE-3 起按阶段接管
```
...
| Web smoke           | `node apps/server/scripts/smoke.mjs`                 | ubuntu CI 冒烟门禁（真启动 apps/server：index.html + assets 可服务、无 token 401、healthCheck ok、数据服务就绪；未本地实测）                                                                                                                                                                              |
