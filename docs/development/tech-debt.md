...
3. ~~**TD-033-3：编辑无自动保存**~~ —— **已解决（2026-08-15，两批：migration v20 + v21）**：
   登记时的顾虑（自动保存推进 currentVersionId 会与 CAS 基线冲突、并撑爆版本历史）
   由**产品决策**消解：自动保存写入**独立草稿层，绝不进版本链**，显式保存仍是唯一
   产生版本的路径，锁定决策"不静默覆盖用户正文"不因 autosave 松动，TD-033-2 刚交付
   的版本列表也不会被淹没。后端 `chapter_drafts`（一章一份草稿，记 baseVersionId 与
   title）+ `manuscript.saveDraft/getDraft/discardDraft` 五层贯通；前端 2000ms 防抖、
   切章取消挂起定时器、显式保存进行中不触发。三条语义各有回归：存草稿不产生版本也
   不动 current 指针；显式保存成功后同事务清草稿（CAS 失败则保留）；current 偏离草稿
   基线时如实回报 stale。**打开章节发现草稿一律不自动套用**，只给横幅让用户选恢复或
   丢弃，stale 时额外说明"正文在此期间已被更新"——避免本项目高发的"要求用户对看不见
   的内容做决定"。MANUSCRIPT_COMMIT（AI 写稿）刻意不清草稿，否则等于替用户静默丢弃
...
**状态**: PARTIALLY_RESOLVED（2026-08-17，B17b）——D12 拍板范围内的最小集已修，
真机系统性审计仍是 OPEN 项，按 D12「桌面为主、移动端顺手看稿」明确不升级优先级。
**优先级**: 移动端体验成为主要使用方式前（未变——D12 未改变这一前提，只是把
「最小集」和「系统审计」拆成了两个不同优先级的条目，见下）
**登记批次**: B13（2026-08-17）→ B17b 复查更新（2026-08-17）
...
### 已解决部分（B17b，390×844 浏览器实测坐实并修复）

- **100vh → 100dvh**：`.app-shell`、TokenGate 全屏卡片，避免 iOS Safari 地址栏
  收放裁底。
- **safe-area-inset**：rail（左/上/下）、抽屉 Sheet（上/右/下）、toast
  （sonner `offset`/`mobileOffset`）三处补 `env(safe-area-inset-*)`，不压刘海/
  Home indicator；非刘海设备 `env()` 恒为 0，数值与之前完全一致。
- **触控目标 ≥44px**：JourneyNav 四个阶段按钮、抽屉关闭钮、头部设置按钮（伪
  元素扩无形命中区，不改可见尺寸，D-B17-2）；TaskList 任务行补 `min-h-11` 地板。
- **两处真实破版**（390px 实测坐实，非推测）：HomePage 首屏两列网格
  `minmax(420px,…)` 硬下限撑破容器（改 `md:` 断点单列/双列切换）；App.tsx 头部
  CJK 标题/设置按钮无截断处理时逐字换行撑破定高 header（补 `truncate`/
  `whitespace-nowrap`/`shrink` 组合）。ProjectListRegion 卡片网格、JourneyNav
  四阶段标签同批补 768px（Tailwind `md`）断点兜底。
- 详见 `b17-cleanup-design.md` §1b、对应 commit message（B17b）。

### 仍未做（明确排除在本批之外，D12 已拍板不做形态改造）

- **真机系统性审计**：iPhone/iPad 真机走查一轮，本批全程用浏览器 390×844 视口
  模拟（含 devtools colorScheme 切换），未上真机；已知与真机的差距——iOS Safari
  实际地址栏收放行为、真实触觉反馈下的命中体验、真实刘海尺寸。
- **iOS Safari Blob 下载面板**：未验证（本批未涉及导出/下载路径）；若不可用，
  兜底 data URL 方案（见 b12 设计 §5）。
- **软键盘遮挡**：访谈/正文编辑等文本输入场景，软键盘弹出时是否遮挡输入框或
  操作按钮，未验证。
- **移动端完整创作流程形态**（底部导航、bottom-sheet、软键盘专项适配）：D12
  已明确拍板不做——当前定位是「顺手看稿」，不是移动端第一优先。

### 后续动作

- 负责人移动端真机走查一轮（本条目最终验收动作，b17-cleanup-design.md §3
  已列入 B17 验收流程），按发现登记具体条目（新条目或本条目追加，视问题大小）
- iPad Safari 下载交互若不可用，兜底 data URL 方案（见 b12 设计 §5）
- 若未来移动端体验优先级上升，「完整创作流程形态」需要单独立项设计（非本条
  目范围，届时记 decision-log）
