...
**故事知识图谱（负责人 2026-08-18 定向，待设计）**：参照 GraphRAG / LightRAG 的思路，为长篇
一致性建一个**从权威正文与 Graph artifacts 派生的故事图结构**——实体（人物/地点/物品/设定）、
关系（人物关系及其随剧情的演变）、事件与时间线、伏笔与未解决线程——逐章抽取增量更新，为后续
章节生成提供检索式上下文（替代「全文塞 prompt」），并作为一致性核验的依据（人物知识范围、
关系状态、时间线冲突）。它是上表 Story State Ledger / 关系与状态 / 时间线 / 伏笔线程的统一
实现载体候选。设计要点（届时单独出设计文档）：抽取用哪一层模型与预算、图存储放 project.sqlite
新表还是独立层、失效传播如何跟随正文修订（用户手改一章后图如何增量重建）、检索接入
CHAPTER_PLAN/DRAFT 的 prompt 组装点、以及与 GE-9 评测装置的结合（图辅助 vs 无图 A/B）。
