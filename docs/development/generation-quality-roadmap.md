...
| Real Generation Experiment Runner    | 🟡   | PR #21（未合并）：source/output suite 语义 + baseline-one-shot-v1 + allowlisted provider registry（MiMo 默认真实模型）+ 目录级原子发布 + secret-store 抽取；Q1 engineering loop=true（v2 maxTokens=8192 COMPLETE 3/3 SUCCEEDED；v1 1024 PARTIAL_FAILURE） |
...
| 2026-08-02 | PR #21（未合并，ready for review）                  | 🟡     | GQ2 Real Generation Experiment Runner                                     | 代码 + 离线测试完成；v1 full suite `PARTIAL_FAILURE`；v2（maxTokens=8192）full suite `COMPLETE`（Q1=true）；**Q1 只表示真实生成→评测→盲评的工程闭环，无质量提升证据、无人工质量结论** | `apps/writing-experiment-runner` + `packages/secret-store`（抽取）、`pnpm check` 2363 passed、allowlisted provider registry（MiMo `mimo-v2.5-pro` 默认真实模型）、source/output suite 语义、目录级原子发布、LIVE gate；**真实实验（7 次付费调用，无自动重试）**：v1 smoke `--max-cases 1` SUCCEEDED（PARTIAL_SELECTION，satisfiesQ1=false）；v1 full `run`（maxTokens=1024）`PARTIAL_FAILURE`（2/3 `PROVIDER_RESPONSE_INVALID`，finishReason `max_tokens`；1 SUCCEEDED），未产生 evaluation/blind；v2 full `run`（maxTokens=8192，budget fix `926704a`）`COMPLETE`，3/3 SUCCEEDED，产出 output suite + evaluation.report + blind.packet + blind.mapping | 人工盲评走 writing-evaluation CLI（validate ratings → aggregate）；GQ2 合并后再更新状态；无文章质量结论 |
...
**GQ2 Runner（PR #21，未合并，🟡）**：已建立真实生成实验的工程代码与离线测试（source/output suite 语义、allowlisted provider registry、MiMo `mimo-v2.5-pro` 为默认真实生成模型、目录级原子发布、manifest、LIVE gate）。**受控真实实验（2026-08-02，用户配置 Keychain 密钥后）**：
...
- **Q1 engineering loop demonstrated with real MiMo candidates. No human quality conclusion exists.** Q1=true 只表示真实生成→评测→盲评 artifact 的工程闭环完成，**不表示文章质量提高**，任何质量提升结论均不成立。GQ2 未合并仍标 🟡（详见 `docs/development/writing-experiment-runner.md`）。
