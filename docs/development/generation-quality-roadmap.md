...
| 2026-08-02 | PR #21（未合并，ready for review）                  | 🟡     | GQ2 Real Generation Experiment Runner                                     | 代码 + 离线测试完成；受控 live smoke 成功（1 真实调用）但 full suite `PARTIAL_FAILURE`；**无质量提升证据；Q1 未达成** | `apps/writing-experiment-runner` + `packages/secret-store`（抽取）、`pnpm check` 2352 passed、allowlisted provider registry（MiMo `mimo-v2.5-pro` 默认真实模型）、source/output suite 语义、目录级原子发布、LIVE gate；**真实实验（4 次付费调用）**：smoke `--max-cases 1` SUCCEEDED（PARTIAL_SELECTION，satisfiesQ1=false）；full `run`（3 case）`PARTIAL_FAILURE`（2/3 `PROVIDER_RESPONSE_INVALID`，finishReason `max_tokens`；1 SUCCEEDED），未产生 evaluation/blind | 评估 max-tokens/thinking 后授权重跑 full suite；人工评分走 writing-evaluation CLI |
...
**GQ2 Runner（PR #21，未合并，🟡）**：已建立真实生成实验的工程代码与离线测试（source/output suite 语义、allowlisted provider registry、MiMo `mimo-v2.5-pro` 为默认真实生成模型、目录级原子发布、manifest、LIVE gate）。**受控真实实验（2026-08-02，用户配置 Keychain 密钥后，共 4 次付费调用）**：live smoke（`--max-cases 1`）成功，产生 1 个真实 candidate（PARTIAL_SELECTION，satisfiesQ1=false）；full `run`（3 case）为 `PARTIAL_FAILURE`——2/3 case `PROVIDER_RESPONSE_INVALID`（finishReason `max_tokens`，MiMo extended-thinking 耗尽 1024-token 输出预算时返回仅 thinking、无 text 的响应），1 个 case SUCCEEDED，未产生 candidates/evaluation/blind。**因此 Q1 工程闭环尚未以「全部 case 真实成功」演示，任何质量提升结论均不成立**。后续：评估提高 `--max-tokens` 或 provider thinking 参数后，在授权下重跑 full suite（见 `docs/development/writing-experiment-runner.md`）。
