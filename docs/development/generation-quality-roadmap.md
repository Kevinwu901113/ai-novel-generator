...
| Active PR                  | #21（feat/gq2-real-generation-runner，draft）                                                    |
...
| Next quality capability    | GQ2 真实 candidate smoke / full suite（需先配置 Keychain 密钥）                                  |
...
| 2026-08-02 | PR #21 / `2ddde07`（未合并，draft）                 | 🟡     | GQ2 Real Generation Experiment Runner                                     | 代码 + 离线测试完成；**无真实生成质量提升证据；Q1 未达成** | `apps/writing-experiment-runner` + `packages/secret-store`（抽取）、`pnpm check` 2352 passed、allowlisted provider registry（MiMo `mimo-v2.5-pro` 为默认真实模型）、source/output suite 语义、目录级原子发布、`ExperimentManifestV1`、LIVE gate；live smoke / full suite 因 **Keychain 未配置密钥 BLOCKED（`LIVE_BLOCKED_KEY_NOT_CONFIGURED`）**，未产生任何 real MiMo candidate | 配置 Keychain 密钥后执行 live smoke（`--max-cases 1`）与全量 `run`；人工评分走 writing-evaluation CLI |
...
**GQ2 Runner（PR #21，未合并，🟡）**：已建立真实生成实验的工程代码与离线测试（source/output suite 语义、allowlisted provider registry、MiMo `mimo-v2.5-pro` 为默认真实生成模型、目录级原子发布、manifest、LIVE gate），但 **live smoke / full suite 尚未执行**——macOS Keychain 未配置 provider 密钥（`LIVE_BLOCKED_KEY_NOT_CONFIGURED`），未产生任何 real MiMo candidate。因此 **Q1 工程闭环尚未以真实 candidate 演示，任何质量提升结论均不成立**。配置 Keychain 密钥后按 `docs/development/writing-experiment-runner.md` 执行 live 阶段。
