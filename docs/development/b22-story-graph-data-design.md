...
- 自动化：两工单合计 55 条新测试全过，`pnpm check` 全链绿。
- 实测（真实 server + 真实模型，夹缝客栈单章项目）：显式保存触发 ✓、
  restoreVersion 触发 ✓（改章失效重抽随之验证：旧边被替换）、
  storyGraph.rebuild ✓（清空计数正确、逐章重抽、空登记表重建收敛）、
  严格解析拦截一次模型垃圾输出（8.9s 短响应判 MODEL_RESPONSE_INVALID
  整体失败，重跑收敛）✓。commit 触发路径未实测（需整条章节生成 run），
  由 worker e2e 测试覆盖，与保存触发共用入队+泵。
- **事故与教训（TD-036 登记）**：`app.sqlite projects.project_directory`
  存**绝对路径**——复制数据根得到的"隔离副本"经索引路径仍指回原数据根，
  隔离验收因此一度写入真实项目（已按应用语义 restoreVersion + rebuild
  完全复原，无数据损失）。该缺陷同时意味着**用户迁移数据根会损坏全部
  项目**。修复方向：project_directory 改存相对 dataRoot 的路径（读取端
  兼容旧绝对路径一个版本）。未修复前任何数据根副本实验必须先改写索引表。
- **前情登记表污染观察**（B23/B24 注意）：登记表里的实体名会诱导模型把
  原文无名角色归并到已知实体（实测：无名掌柜被挂到登记表遗留的杜撰名下）。
  rebuild 是净化手段；B24 故事圣经 UI 的实体编辑/删除要考虑「污染实体
  参与后续章指代」的传播效应。
