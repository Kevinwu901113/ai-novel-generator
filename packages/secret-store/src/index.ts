/**
 * @ai-novel/secret-store
 *
 * macOS Keychain SecretStore 唯一实现（/usr/bin/security adapter）。
 *
 * 本 package 是仓库内唯一承载 `/usr/bin/security` 调用逻辑的实现。
 * `apps/worker/src/secret-store.ts` 与 `apps/writing-experiment-runner`
 * 都只 re-export / 消费本实现，不存在第二份实现副本。
 *
 * 安全要求（行为与迁移前完全一致）：
 * - execFile（不用 shell 拼接，不阻塞事件循环）
