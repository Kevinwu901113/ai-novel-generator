...
export const PROJECT_MIGRATIONS: ReadonlyArray<Migration> = [
  ...
  },
  {
    version: 13,
    sql: `
      -- ── 重建 tasks 表：task_type CHECK 加入 SPEC_EXTRACT（GE-3 / B3）──
      -- 镜像 v11 重建模式：创建 tasks_new → 复制 → 删除 → 重命名 → 重建索引。
      CREATE TABLE tasks_new (
        id TEXT PRIMARY KEY,
        project_id TEXT NOT NULL,
        task_type TEXT NOT NULL,
        ...
        stale_at TEXT,
        cancelled_at TEXT,
        CHECK (task_type IN ('PROVIDER_CONNECTION_TEST', 'MODEL_INVOCATION_TEST', 'GRILL_QUESTION_PLAN', 'CREATION_CONTRACT_DRAFT', 'CHAPTER_DRAFT', 'SPEC_EXTRACT')),
        CHECK (status IN ('PENDING', 'RUNNING', 'SUCCEEDED', 'FAILED', 'CANCELLED', 'STALE')),
        CHECK (attempt_count >= 0),
        CHECK (json_valid(input_version_json)),
        CHECK (json_valid(payload_json)),
  ...
  },
  {
    version: 14,
    sql: `
      -- ── 重建 tasks 表：task_type CHECK 加入 RESEARCH_RUN（GE-4 / B5）──
      -- 镜像 v13 重建模式：创建 tasks_new → 复制 → 删除 → 重命名 → 重建索引。
      CREATE TABLE tasks_new (
        id TEXT PRIMARY KEY,
        project_id TEXT NOT NULL,
        task_type TEXT NOT NULL,
        ...
        stale_at TEXT,
        cancelled_at TEXT,
        CHECK (task_type IN ('PROVIDER_CONNECTION_TEST', 'MODEL_INVOCATION_TEST', 'GRILL_QUESTION_PLAN', 'CREATION_CONTRACT_DRAFT', 'CHAPTER_DRAFT', 'SPEC_EXTRACT', 'RESEARCH_RUN')),
        CHECK (status IN ('PENDING', 'RUNNING', 'SUCCEEDED', 'FAILED', 'CANCELLED', 'STALE')),
        CHECK (attempt_count >= 0),
        CHECK (json_valid(input_version_json)),
        CHECK (json_valid(payload_json)),
  ...
  },
  {
    version: 16,
    sql: `
      -- ── 重建 tasks 表：task_type CHECK 加入 BLUEPRINT_GENERATE（GE-5 / B7）──
      -- 镜像 v13/v14 重建模式：创建 tasks_new → 复制 → 删除 → 重命名 → 重建索引。
      CREATE TABLE tasks_new (
        id TEXT PRIMARY KEY,
        project_id TEXT NOT NULL,
        task_type TEXT NOT NULL,
        ...
        stale_at TEXT,
        cancelled_at TEXT,
        CHECK (task_type IN ('PROVIDER_CONNECTION_TEST', 'MODEL_INVOCATION_TEST', 'GRILL_QUESTION_PLAN', 'CREATION_CONTRACT_DRAFT', 'CHAPTER_DRAFT', 'SPEC_EXTRACT', 'RESEARCH_RUN', 'BLUEPRINT_GENERATE')),
        CHECK (status IN ('PENDING', 'RUNNING', 'SUCCEEDED', 'FAILED', 'CANCELLED', 'STALE')),
        CHECK (attempt_count >= 0),
        CHECK (json_valid(input_version_json)),
        CHECK (json_valid(payload_json)),
